...
public class AbstractTestOps extends AbstractTestAPI{
	...
	public void assertContent(String content) throws Exception{
		...
		MicroBlogInfo info = getMicroBlogAPI().add(getAccessToken(), content);
		...
		MicroBlogInfo repost = getMicroBlogAPI().repost(getAccessToken(), id, "--repost--" + new Date().getTime());
		...
		MicroBlogCommentInfo comment = getMicroBlogAPI().comment(getAccessToken(), id, "--comment--" + new Date().getTime());
