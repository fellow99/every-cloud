...
$(document).ready(function(){
	$.ajax({type:"POST",
		...
		success: function(data, status){
			...
			$("#div-menu-provider").setParam("method", "BLOG");
...
function subProviderClick(subProvider){
	api.blog.list(subProvider, "public");
}


api = {};
api.blog = {};
api.blog.list = function(provider, method, page, size){
	if(!page)page=0;
	if(!size)size=20;
	var url = "../api/blog-data.do?method="+method+"&provider="+provider+"&page="+page+"&size="+size;
	$.ajax({type:"POST",
		async:false,
		url: url,
		dataType:"json",
		success: function(data, status){
			$("#div-content").setTemplateURL("../_html/blog-list.html");
